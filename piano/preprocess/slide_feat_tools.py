# Feature extraction from patches, Shape from Image patches -> [N, C]
...
def func_feat_ext(args, pair_list, gpu_id):
    ...
    model = create_patch_encoder(args.model_name, args.ckpt, local_dir=args.local_dir)
    model.eval()
    model.to(device)
        ...
        try:
            ...
            with torch.inference_mode():
                ...
                for batch_img, batch_coord in data_loader:
                    ...
                    features.append(patch_feat.to(amp_dtype).cpu())
                    coordinates.append(batch_coord)
                    pbar.update(len(batch_img))

                feature_box = torch.cat(features, dim=0)
                coord_box = torch.cat(coordinates, dim=0)
                
                pbar.set_description(f'GPU {gpu_id} Slide {item+1}/{len(pair_list)} | Shape {feature_box.shape}')
                time.sleep(1)
