...
def planar_metrics(logits, labels, num_classes):       # logits:[batch_size, num_classes]   labels:[batch_size, ]
    ...
    # macro-average area under the curve (AUC) scores
...
def survival_metrics(all_censorships, all_event_times, all_risk_scores):
    """
    Calculate survival analysis metrics using scikit-survival
    
    Args:
        all_censorships (np.ndarray): Censorship status (0=censored, 1=event)
        all_event_times (np.ndarray): Event times
        all_risk_scores (np.ndarray): Risk scores calculated from survival probabilities
    
    Returns:
        dict: Dictionary containing survival analysis metrics
    """
    
    # Calculate C-index using scikit-survival with the same parameters as user's code
