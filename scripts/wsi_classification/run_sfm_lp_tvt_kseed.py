...
from piano.utils.wsi_finetune_tools import train_wsi, predict_wsi, wsi_create_ckpt, wsi_load_ckpt, SFMClassifier
...
def parse_args():
    ...
    parser.add_argument('--pfm_name', type=str, default='chief', help='Pathology foundation model (slide-level) name')
    parser.add_argument('--ft_mode', type=str, default='lp', choices=['lp', 'mlp'], help='Training mode')
...
def setup_experiment_folder(args, seed=None):
    ...
    main_exp_name = f"KSeed_{args.pfm_name}_{args.ft_mode}"
...
def train_single_seed(args, seed, device):
    ...
    train_dataset = OneSlideDataset(args.data_json, mode='train', pfm_name=args.pfm_name, few_shot=args.few_shot)
    val_dataset = OneSlideDataset(args.data_json, mode='valid', pfm_name=args.pfm_name)
    
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4)
    
    # Create model
    print(f"Creating \033[95mLinear Probe\033[0m fine-tuning on slide-level foundation model \033[91m{args.pfm_name}\033[0m...")
    # Get feature dimension from first batch of training data
    sample_batch = next(iter(train_loader))
    feat_dim = sample_batch['features'].shape[-1]
    print(f"\033[95mFeature dimension: {feat_dim}\033[0m")

    classifier = SFMClassifier(input_dim=feat_dim, num_classes=len(train_dataset.get_classes()), training_mode=args.ft_mode)
    ...
    for epoch in range(args.num_epochs):
        ...
        if current_metric > best_metric:
            ...
            # Save best model
            checkpoint = wsi_create_ckpt(model, args.ft_mode, epoch)
            save_name = f"best_{args.save_metric}_{args.pfm_name}_{args.ft_mode}.pth"
        ...
        if (epoch + 1) % args.save_interval == 0:
            checkpoint = wsi_create_ckpt(model, args.ft_mode, epoch)
            save_name = f"{args.pfm_name}_{args.ft_mode}_epoch{epoch+1}.pth"
...
def test_single_seed(args, seed, device, model_path, exp_dir):
    ...
    test_dataset = OneSlideDataset(args.data_json, mode='test', pfm_name=args.pfm_name)
    ...
    # Get feature dimension from first batch of test data
    sample_batch = next(iter(test_loader))
    feat_dim = sample_batch['features'].shape[-1]
    
    classifier = SFMClassifier(input_dim=feat_dim, num_classes=num_classes, training_mode=args.ft_mode)
    model = classifier.to(device)
