...
from piano.utils.wsi_finetune_tools import train_wsi, predict_wsi, wsi_create_ckpt, SFMClassifier
...
def setup_experiment_folder(args, fold_idx=None):
    ...
    main_exp_name = f"Kfold_{args.pfm_name}_{args.ft_mode}"
...
def train_single_fold(args, fold_idx, device):
    ...
    train_dataset = OneSlideDatasetKFold(args.data_json, fold_idx, mode='train', pfm_name=args.pfm_name, few_shot=args.few_shot)
    valid_dataset = OneSlideDatasetKFold(args.data_json, fold_idx, mode='valid', pfm_name=args.pfm_name)
    ...
    save_predictions(best_val_logits, best_val_labels, valid_dataset, exp_dir)
